...
假设上面的操作记录的`USB port`数值分别为`3-1.4:1.0`、`3-1.1:1.0`，则将下面的`USB_PORTS["1-9:1.0"]="can_left_1:1000000"`的中括号内部的双引号内部的参数换为`3-1.4:1.0`和`3-1.1:1.0`.
...
解释：**3-1.4:1.0硬件编码的usb端口插入的can设备，名字被重命名为can_left_1，波特率为1000000，并激活**
...
执行`ifconfig`查看是不是有`can_left_1`和`can_right_1`
