...
parser.add_argument("--can_port", type=str, default="can_right_1", help="CAN port name")
parser.add_argument(
    "--hz", type=float, default=10, help="Refresh rate (Hz), range: 0.5 ~ 200"
)
    ...
    while not exit_flag:
        ...
        if start_time - last_time >= limit_interval:
            if args.req_flag == 1:
                ...
                piper.ArmParamEnquiryAndConfig(
                    param_enquiry=0x02,
                    param_setting=0x00,
        ...
        print(f"+{'='*107}+")
        print(
            f"Firmware Ver : {piper.GetPiperFirmwareVersion():<10}"
            f"\n"
