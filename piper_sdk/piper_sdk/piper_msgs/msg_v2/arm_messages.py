...
# ----------------------------------基于V1.5-2版本后---------------------------------------------#
from .feedback.arm_feedback_gripper_teaching_param import (
...
from .feedback.arm_feedback_crash_protection_rating import (
...
from .transmit.arm_gripper_teaching_param_config import (
...
class PiperMessage:
    ...
    def __init__(
        ...
        self.type_ = type_
        # 初始化机械臂状态消息
        self.arm_status_msgs = (
            arm_status_msgs if arm_status_msgs else ArmMsgFeedbackStatus()
        )
        # 初始化机械臂关节反馈
        self.arm_joint_feedback = (
            arm_joint_feedback if arm_joint_feedback else ArmMsgFeedBackJointStates()
        )
        # 初始化夹爪反馈
        self.gripper_feedback = (
            gripper_feedback if gripper_feedback else ArmMsgFeedBackGripper()
        )
        # 初始化末端姿态反馈
        self.arm_end_pose = arm_end_pose if arm_end_pose else ArmMsgFeedBackEndPose()
        # 驱动器信息高速反馈
        self.arm_high_spd_feedback_1 = (
            arm_high_spd_feedback if arm_high_spd_feedback else ArmMsgFeedbackHighSpd()
        )
        ...
        self.arm_low_spd_feedback_1 = (
            arm_low_spd_feedback if arm_low_spd_feedback else ArmMsgFeedbackLowSpd()
        )
        ...
        self.arm_gripper_teaching_param_feedback = (
            ...
            if arm_gripper_teaching_param_feedback
            else ArmMsgFeedbackGripperTeachingPendantParam()
        )
        # -------------------------------发送-------------------------------------------
        ...
        # 关节mit控制
        self.arm_joint_mit_ctrl = (
            arm_joint_mit_ctrl if arm_joint_mit_ctrl else ArmMsgJointMitCtrl()
        )
        self.arm_ms_config = (
        ...
        self.arm_crash_protection_rating_feedback = (
            ...
            else ArmMsgFeedbackCrashProtectionRating()
        )
        # 反馈当前电机最大加速度限制
        ...
        # 夹爪/示教器参数设置指令
        self.arm_gripper_teaching_param_config = (
            arm_gripper_teaching_param_config
            if arm_gripper_teaching_param_config
            else ArmMsgGripperTeachingPendantParamConfig()
        )
        # 反馈各个关节当前末端速度/加速度
