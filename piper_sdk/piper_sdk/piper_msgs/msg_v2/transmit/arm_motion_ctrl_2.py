...
class ArmMsgMotionCtrl_2:
    ...
    def __init__(
        ...
        if ctrl_mode not in [0x00, 0x01, 0x03, 0x04, 0x07]:
            raise ValueError(
                f"'ctrl_mode' Value {ctrl_mode} out of range [0x00, 0x01, 0x02, 0x03, 0x04, 0x07]"
            )
        if move_mode not in [0x00, 0x01, 0x02, 0x03, 0x04]:
            raise ValueError(
                f"'move_mode' Value {move_mode} out of range [0x00, 0x01, 0x02, 0x03, 0x04]"
            )
        if not (0 <= move_spd_rate_ctrl <= 100):
            raise ValueError(
                f"'move_spd_rate_ctrl' Value {move_spd_rate_ctrl} out of range 0-100"
            )
        if mit_mode not in [0x00, 0xAD, 0xFF]:
            raise ValueError(
                f"'mit_mode' Value {mit_mode} out of range [0x00, 0xAD, 0xFF]"
            )
        if not (0 <= residence_time <= 255):
            raise ValueError(
                f"'residence_time' Value {residence_time} out of range 0-255"
            )
        if installation_pos not in [0x00, 0x01, 0x02, 0x03]:
            raise ValueError(
                f"'installation_pos' Value {installation_pos} out of range [0x00, 0x01, 0x02, 0x03]"
            )
