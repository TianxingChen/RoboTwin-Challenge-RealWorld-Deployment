...
class ArmMsgMotionCtrl_1:
    ...
    def __init__(
        ...
        if emergency_stop not in [0x00, 0x01, 0x02]:
            raise ValueError(
                f"'emergency_stop' Value {emergency_stop} out of range [0x00, 0x01, 0x02]"
            )

        # 检查 track_ctrl 是否在有效范围内
        if track_ctrl not in [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]:
            raise ValueError(
                f"'track_ctrl' Value {track_ctrl} out of range [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]"
            )

        # 检查 grag_teach_ctrl 是否在有效范围内
        if grag_teach_ctrl not in [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07]:
            raise ValueError(
                f"'grag_teach_ctrl' Value {grag_teach_ctrl} out of range [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07]"
            )
