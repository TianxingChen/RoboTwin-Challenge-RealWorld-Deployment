...
class ArmMsgMasterSlaveModeConfig:
    ...
    def __init__(
        ...
        if linkage_config not in [0x00, 0xFA, 0xFC]:
            raise ValueError(
                f"'linkage_config' Value {linkage_config} out of range [0x00, 0xFA, 0xFC]"
            )
        if feedback_offset not in [0x00, 0x10, 0x20]:
            raise ValueError(
                f"'feedback_offset' Value {feedback_offset} out of range [0x00, 0x10, 0x20]"
            )
        if ctrl_offset not in [0x00, 0x10, 0x20]:
            raise ValueError(
                f"'ctrl_offset' Value {ctrl_offset} out of range [0x00, 0x10, 0x20]"
            )
        if linkage_offset not in [0x00, 0x10, 0x20]:
            raise ValueError(
                f"'linkage_offset' Value {linkage_offset} out of range [0x00, 0x10, 0x20]"
            )
