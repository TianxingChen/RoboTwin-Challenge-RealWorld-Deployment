...
class ArmMsgParamEnquiryAndConfig:
    ...
    def __init__(
        ...
        if param_enquiry not in [0x00, 0x01, 0x02, 0x03]:
            raise ValueError(
                f"'param_enquiry' Value {param_enquiry} out of range [0x00, 0x01, 0x02, 0x03]"
            )
        if param_setting not in [0x00, 0x01, 0x02]:
            raise ValueError(
                f"'param_setting' Value {param_setting} out of range [0x00, 0x01, 0x02]"
            )
        if data_feedback_0x48x not in [0x00, 0x01, 0x02]:
            raise ValueError(
                f"'data_feedback_0x48x' Value {data_feedback_0x48x} out of range [0x00, 0x01, 0x02]"
            )
        if end_load_param_setting_effective not in [0x00, 0xAE]:
            raise ValueError(
                f"'end_load_param_setting_effective' Value {end_load_param_setting_effective} out of range [0x00, 0xAE]"
            )
        if set_end_load not in [0x00, 0x01, 0x02, 0x03]:
            raise ValueError(
                f"'set_end_load' Value {set_end_load} out of range [0x00, 0x01, 0x02, 0x03]"
            )
