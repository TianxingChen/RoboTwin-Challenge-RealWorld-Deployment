...
class ArmMsgJointConfig:
    ...
    Args:
        ...
        max_joint_acc: 最大关节加速度,单位0.01rad/s^2,输入范围\[0, 500\]-->[0 rad/s^2, 5.0 rad/s^2]
    ...
    Args:
        ...
        max_joint_acc: Maximum joint acceleration, unit: 0.01rad/s^2.Range is \[0, 500\]-->[0 rad/s^2, 5.0 rad/s^2]
    ...
    def __init__(
        ...
        if joint_motor_num not in [1, 2, 3, 4, 5, 6, 7]:
            raise ValueError(
                f"'joint_motor_num Value' {joint_motor_num} out of range [1, 2, 3, 4, 5, 6, 7]"
            )
        if set_motor_current_pos_as_zero not in [0x00, 0xAE]:
            raise ValueError(
                f"'set_motor_current_pos_as_zero' Value {set_motor_current_pos_as_zero} out of range [0x00, 0xAE]"
            )
        if acc_param_config_is_effective_or_not not in [0x00, 0xAE]:
            raise ValueError(
                f"'acc_param_config_is_effective_or_not' Value {acc_param_config_is_effective_or_not} out of range [0x00, 0xAE]"
            )
        if not (0 <= max_joint_acc <= 500):
            raise ValueError(
                f"'max_joint_acc' Value {max_joint_acc} out of range 0-500."
            )
        if clear_joint_err not in [0x00, 0xAE]:
            raise ValueError(
                f"clear_joint_err' Value {clear_joint_err} out of range [0x00, 0xAE]"
            )
