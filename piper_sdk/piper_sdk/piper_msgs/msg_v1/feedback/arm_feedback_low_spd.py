...
class ArmMsgFeedbackLowSpd:
    ...
    def __init__(
        ...
        if can_id not in [0x000, 0x261, 0x262, 0x263, 0x264, 0x264, 0x265, 0x266]:
            raise ValueError(
                f"'can_id' Value {can_id} out of range [0x000, 0x261, 0x262, 0x263, 0x264, 0x264, 0x265, 0x266]"
            )
        self.can_id = can_id
        self.vol = vol
    ...
    def foc_status_code(self, value: int):
        ...
        self.foc_status.collision_status = bool(value & (1 << 4))
        self.foc_status.driver_error_status = bool(value & (1 << 5))
        self.foc_status.driver_enable_status = bool(value & (1 << 6))
        self.foc_status.stall_status = bool(value & (1 << 7))

    def __str__(self):
        return (
            f"ArmMsgFeedbackLowSpd(\n"
            f"  can_id: {hex(self.can_id)}\n"
            f"  vol: {self.vol}\n"
            f"  foc_temp: {self.foc_temp }\n"
            f"  motor_temp: {self.motor_temp }\n"
            f"  foc_status: \n{self.foc_status }\n"
            f"  bus_current: {self.bus_current}\n"
            f")"
        )
