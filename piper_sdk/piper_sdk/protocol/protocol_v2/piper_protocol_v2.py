...
from ...piper_msgs.msg_v2 import ArmMsgType, PiperMessage, CanIDPiper, ArmMessageMapping
...
class C_PiperParserV2(C_PiperParserBase):
    ...
    def DecodeMessage(self, rx_can_frame: Optional[can.Message], msg: PiperMessage):
        ...
        # 驱动器信息高速反馈,High-Speed Driver Information Feedback
            ...
            msg.arm_high_spd_feedback_2.pos = self.ConvertToNegative_32bit(
                self.ConvertBytesToInt(can_data, 4, 8)
            )
    ...
    def EncodeMessage(self, msg: PiperMessage, tx_can_frame: Optional[can.Message]):
        ...
        if msg_type_ == ArmMsgType.PiperMsgMotionCtrl_1:
            ...
            tx_can_frame.data = (
                ...
                + self.ConvertToList_16bit(
                    msg.arm_motor_angle_limit_max_spd_set.max_joint_spd, False
                )
                + [0]
            )
            ...
            tx_can_frame.data = (
                self.ConvertToList_16bit(
                    msg.arm_end_vel_acc_param_config.end_max_linear_vel, False
                )
                + self.ConvertToList_16bit(
                    msg.arm_end_vel_acc_param_config.end_max_angular_vel, False
                )
                + self.ConvertToList_16bit(
                    msg.arm_end_vel_acc_param_config.end_max_linear_acc, False
                )
                + self.ConvertToList_16bit(
                    msg.arm_end_vel_acc_param_config.end_max_angular_acc, False
                )
            )
        elif msg_type_ == ArmMsgType.PiperMsgCrashProtectionRatingConfig:
            tx_can_frame.data = (
                self.ConvertToList_8bit(
                    msg.arm_crash_protection_rating_config.joint_1_protection_level,
                    False,
                )
                + self.ConvertToList_8bit(
                    msg.arm_crash_protection_rating_config.joint_2_protection_level,
                    False,
                )
                + self.ConvertToList_8bit(
                    msg.arm_crash_protection_rating_config.joint_3_protection_level,
                    False,
                )
                + self.ConvertToList_8bit(
                    msg.arm_crash_protection_rating_config.joint_4_protection_level,
                    False,
                )
                + self.ConvertToList_8bit(
                    msg.arm_crash_protection_rating_config.joint_5_protection_level,
                    False,
                )
                + self.ConvertToList_8bit(
                    msg.arm_crash_protection_rating_config.joint_6_protection_level,
                    False,
                )
                + [0, 0]
            )
            ...
            tx_can_frame.data = (
                self.ConvertToList_8bit(
                    msg.arm_gripper_teaching_param_config.teaching_range_per, False
                )
                + self.ConvertToList_8bit(
                    msg.arm_gripper_teaching_param_config.max_range_config, False
                )
                + self.ConvertToList_8bit(
                    msg.arm_gripper_teaching_param_config.teaching_friction, False
                )
                + [0, 0, 0, 0, 0]
            )
        # 机械臂MIT单独控制电机
        elif (
            msg_type_ == ArmMsgType.PiperMsgJointMitCtrl_1
            or msg_type_ == ArmMsgType.PiperMsgJointMitCtrl_2
            or msg_type_ == ArmMsgType.PiperMsgJointMitCtrl_3
            or msg_type_ == ArmMsgType.PiperMsgJointMitCtrl_4
            or msg_type_ == ArmMsgType.PiperMsgJointMitCtrl_5
            or msg_type_ == ArmMsgType.PiperMsgJointMitCtrl_6
        ):
            tx_can_frame.data = (
                self.ConvertToList_16bit(msg.arm_joint_mit_ctrl.pos_ref, False)
                + self.ConvertToList_8bit(
                    ((msg.arm_joint_mit_ctrl.vel_ref >> 4) & 0xFF), False
                )
                + self.ConvertToList_8bit(
                    (
                        (((msg.arm_joint_mit_ctrl.vel_ref & 0xF) << 4) & 0xF0)
                        | ((msg.arm_joint_mit_ctrl.kp >> 8) & 0x0F)
                    ),
                    False,
                )
                + self.ConvertToList_8bit(msg.arm_joint_mit_ctrl.kp & 0xFF, False)
                + self.ConvertToList_8bit(
                    (msg.arm_joint_mit_ctrl.kd >> 4) & 0xFF, False
                )
                + self.ConvertToList_8bit(
                    (
                        (((msg.arm_joint_mit_ctrl.kd & 0xF) << 4) & 0xF0)
                        | ((msg.arm_joint_mit_ctrl.t_ref >> 4) & 0x0F)
                    ),
                    False,
                )
            )
            ...
            tx_can_frame.data = tx_can_frame.data + self.ConvertToList_8bit(
                (((msg.arm_joint_mit_ctrl.t_ref << 4) & 0xF0) | crc), False
            )
        else:
            ret = False
        return ret
