...
class C_PiperParserBase(ABC):
    ...
    def ConvertToNegative_8bit(self, value: int, signed: bool = True) -> int:
        ...
        if not (0 <= value <= 255):
            print(
                "Error ConvertToNegative_8bit:  Input value exceeds the range [0, 255]."
            )
        # 转换成 8 位无符号整数
    ...
    def ConvertToNegative_int8_t(value: int) -> int:
        ...
        if not (0 <= value <= 255):
            print(
                "Error ConvertToNegative_int8_t: Input value exceeds the range [0, 255]."
            )
        # 转换成 8 位无符号整数
        value &= 0xFF  # 将 value 转换成 8 位无符号整数
        if value & 0x80:  # 检查符号位
    ...
    def ConvertToNegative_uint8_t(value: int) -> int:
        ...
        if not (0 <= value <= 255):
            print(
                "Error ConvertToNegative_uint8_t: Input value exceeds the range [0, 255]."
            )
        # 转换成 8 位无符号整数
        value &= 0xFF  # 将 value 转换成 8 位无符号整数
        return value
    ...
    def ConvertToNegative_16bit(self, value: int, signed: bool = True) -> int:
        ...
        if not (0 <= value <= 65535):
            print(
                "Error ConvertToNegative_16bit: Input value exceeds the range [0, 65535]."
            )
        # 转换成 16 位无符号整数
    ...
    def ConvertToNegative_int16_t(value: int) -> int:
        ...
        if not (0 <= value <= 65535):
            print(
                "Error ConvertToNegative_int16_t: Input value exceeds the range [0, 65535]."
            )
        # 转换成 8 位无符号整数
    ...
    def ConvertToNegative_uint16_t(value: int) -> int:
        ...
        if not (0 <= value <= 65535):
            print(
                "Error ConvertToNegative_uint16_t: Input value exceeds the range [0, 65535]."
            )
        # 转换成 8 位无符号整数
        value &= 0xFFFF  # 将 value 转换成 8 位无符号整数
        return value
    ...
    def ConvertToNegative_32bit(self, value: int, signed: bool = True):
        ...
        if not (0 <= value <= 4294967295):
            print(
                "Error ConvertToNegative_32bit: Input value exceeds the range [0, 4294967295]."
            )
        # 转换成 32 位无符号整数
    ...
    def ConvertToList_8bit(self, value: int, signed: bool = True):
        ...
        if signed:
            if not -128 <= value <= 127:
                raise OverflowError(
                    f"The input value {value} exceeds the range of an 8-bit signed integer [-128, 127]."
                )
            ...
            if not 0 <= value <= 255:
                raise OverflowError(
                    f"The input value {value} exceeds the range of an 8-bit unsigned integer [0, 255]."
                )
            return list(struct.unpack("B", struct.pack(">B", value)))
    ...
    def ConvertToList_16bit(self, value: int, signed: bool = True):
        ...
        if signed:
            if not -32768 <= value <= 32767:
                raise OverflowError(
                    f"The input value {value} exceeds the range of a 16-bit signed integer [-32768, 32767]."
                )
            ...
            if not 0 <= value <= 65535:
                raise OverflowError(
                    f"The input value {value} exceeds the range of a 16-bit unsigned integer [0, 65535]."
                )
            return list(struct.unpack("BB", struct.pack(">H", value)))
    ...
    def ConvertToList_32bit(self, value: int, signed: bool = True):
        ...
        if signed:
            if not -2147483648 <= value <= 2147483647:
                raise OverflowError(
                    f"The input value {value} exceeds the range of a 32-bit signed integer [-2147483648, 2147483647]."
                )
            ...
            if not 0 <= value <= 4294967295:
                raise OverflowError(
                    f"The input value {value} exceeds the range of a 32-bit unsigned integer [0, 4294967295]."
                )
            return list(struct.unpack("BBBB", struct.pack(">I", value)))
