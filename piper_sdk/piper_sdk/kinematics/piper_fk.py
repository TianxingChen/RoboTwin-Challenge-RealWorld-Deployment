...
class C_PiperForwardKinematics:
    ...
    def __MatrixToeula(self, T):
        ...
        if T[8] < -1 + 0.0001:
            ...
            _bt = math.atan2(
                -T[8], math.sqrt(T[0] * T[0] + T[4] * T[4])
            )  # pitch (beta)
            Pos[4] = _bt * self.RADIAN
            Pos[5] = (
                math.atan2(T[4] / math.cos(_bt), T[0] / math.cos(_bt)) * self.RADIAN
            )  # yaw (gamma)
            Pos[3] = (
                math.atan2(T[9] / math.cos(_bt), T[10] / math.cos(_bt)) * self.RADIAN
            )  # roll (alpha)
    ...
    def CalFK(self, cur_j):
        ...
        for i in range(6):
            ...
            _Rt[i] = self.__LinkTransformtion(
                self._alpha[i], self._a[i], c_theta, self._d[i]
            )
