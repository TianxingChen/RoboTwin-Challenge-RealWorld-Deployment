...
class C_PiperParamManager:
    ...
    def GetJointLimitParam(
        self, joint_name: Literal["j1", "j2", "j3", "j4", "j5", "j6"]
    ):
        if joint_name not in ["j1", "j2", "j3", "j4", "j5", "j6"]:
            raise ValueError(
                f'"joint_name" Value {joint_name} is not in ["j1", "j2", "j3", "j4", "j5", "j6"]'
            )
        return (
            self.PIPER_PARAM["joint_limit"][joint_name][0],
            self.PIPER_PARAM["joint_limit"][joint_name][1],
        )

    def GetGripperRangeParam(self):
        return (
            self.PIPER_PARAM["gripper_range"][0],
            self.PIPER_PARAM["gripper_range"][1],
        )

    def SetJointLimitParam(
        self,
    ...
    def SetGripperRangeParam(self, min_val: float, max_val: float):
        if max_val - min_val < 0:
            raise ValueError(f"max_val should be greater than min_val.")
